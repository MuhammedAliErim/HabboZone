...
export default function StaffClient({ 
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="habbo-box bg-[#0a1224] border-2 border-white/10 rounded-2xl p-6 shadow-2xl">
        <div className="flex items-center justify-between">
          ...
          {!isAdding && (
            <button 
              ...
              className="habbo-button bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-2 transition-all shadow-lg shadow-blue-500/20"
            >
        ...
        {isAdding && (
          <div className="space-y-6 border-t border-white/10 pt-6 mt-6 animate-in slide-in-from-top-2">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <select 
                  ...
                  className="habbo-box w-full bg-[#050a14] border-2 border-white/10 text-white rounded-xl px-4 py-3 text-sm font-bold focus:border-blue-500 focus:outline-none"
                >
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button 
                onClick={handleAdd}
                className="habbo-button bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-2 transition-all shadow-lg"
                disabled={loading}
              >
                    ...
                    <td className="px-6 py-4">
                      {editingId === s.id ? (
                        <input 
                          ...
                          className="habbo-box bg-[#050a14] border-2 border-blue-500 text-white rounded-xl px-3 py-1.5 text-sm font-bold focus:outline-none w-full max-w-[220px]"
                          autoFocus
                        />
                      ) : (
                        <span className="font-bold text-yellow-300 bg-yellow-400/5 border border-yellow-400/20 px-3 py-1.5 rounded-xl text-xs">
                          {s.position}
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4">
                       {editingId === s.id ? (
                        <input 
                          ...
                          className="habbo-box bg-[#050a14] border-2 border-blue-500 text-white rounded-xl px-3 py-1.5 text-sm font-bold focus:outline-none w-20"
                        />
                      ) : (
                        <span className="font-black text-gray-400 bg-white/5 px-2.5 py-1 rounded-lg text-xs">
                          #{s.order_index}
                        </span>
                      )}
                    </td>
                      ...
                      {editingId === s.id ? (
                        <div className="flex items-center justify-end gap-2">
                          <button 
                            ...
                            disabled={loading}
                            className="p-2 bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-white rounded-xl transition-all font-bold"
                          ...
                          <button 
                            ...
                            disabled={loading}
                            className="p-2 bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white rounded-xl transition-all"
                            title="İptal Et"
                          >
                            <X size={16} />
                          </button>
