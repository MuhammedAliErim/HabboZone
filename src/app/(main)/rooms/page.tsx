...
export default async function RoomsPage() {
  ...
  // Fallback to empty array if no rooms
      ...
      <div className="max-w-[1000px] mx-auto px-6">
        ...
        <div className="space-y-3">
            {displayRooms.length > 0 ? (
              displayRooms.map((room) => (
                  <Link href="#" key={room.id} className="habbo-box habbo-card-hover p-3 flex items-center gap-4 group">
                      ...
                      <div className="w-[120px] h-[80px] shrink-0 rounded overflow-hidden relative border border-[#1e293b]">
                          ...
                          <span className="absolute top-1 left-1 bg-black/60 backdrop-blur-sm px-1.5 py-0.5 rounded text-[9px] font-bold text-white border border-white/10 uppercase">
                            {room.category}
                          </span>
                      </div>
                      ...
                      <div className="flex-1 flex flex-col justify-center min-w-0">
                          ...
                          <p className="text-[12px] text-[#94a3b8] line-clamp-1">{room.description}</p>
                      </div>
              ...
              <div className="text-center py-12 bg-[#0a1325] rounded-md border border-[#1e293b]">
                <h3 className="text-gray-400 font-bold">Henüz hiç oda bulunamadı.</h3>
              </div>
            )}
        </div>
