...
export default function EditNewsPage({ params }: { params: { id: string } }) {
  ...
  const fetchData = async () => {
    ...
    // Fetch categories
    const { data: cats } = await supabase.from('categories').select('id, name').eq('type', 'news');
    if (cats) {
      setCategories(cats);
    }
    ...
    if (newsItem) {
      ...
      setCategoryId(newsItem.category_id || (cats && cats[0]?.id) || '');
      setStatus(newsItem.status);
      setExistingImageUrl(newsItem.thumbnail_url || '');
      
      if (newsItem.seo_metadata) {
        setMetaTitle(newsItem.seo_metadata.meta_title || '');
        setMetaDescription(newsItem.seo_metadata.meta_description || '');
      }
    }
    setPageLoading(false);
  };
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          published_at: status === 'Published' && status !== 'Published' ? new Date().toISOString() : undefined,
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20 p-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="text-3xl font-black mb-1 uppercase tracking-widest">Haberi Düzenle</h2>
          <p className="text-white/60">İçeriği güncelle ve hemen yayınla.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white/5 border-2 border-white/10 rounded-2xl p-6 space-y-6">
          ...
          <div>
            ...
            {existingImageUrl && !coverImage && (
              <div className="mb-4">
                <p className="text-sm opacity-60 mb-2">Mevcut Resim:</p>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={existingImageUrl} alt="Kapak" className="h-32 object-cover rounded-lg border-2 border-white/10" />
              </div>
            )}
            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-white/20 border-dashed rounded-lg cursor-pointer bg-black/20 hover:bg-white/5 transition-colors">
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                <ImageIcon className="w-8 h-8 mb-2 opacity-50" />
                <p className="mb-1 text-sm text-white/60 font-bold">
                  {coverImage ? coverImage.name : 'Yeni resim yüklemek için tıklayın (Eskisini değiştirir)'}
                </p>
              </div>
        ...
        <div className="bg-white/5 border-2 border-white/10 rounded-2xl p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-bold mb-2 opacity-80 uppercase text-blue-400">SEO Meta Title</label>
              <input
                type="text"
                value={metaTitle}
          ...
          <div>
            <label className="block text-sm font-bold mb-2 opacity-80 uppercase text-blue-400">SEO Meta Description</label>
            <textarea
              rows={2}
