    ...
    <div className="group habbo-box bg-[#0a1325] border border-[#1e293b] rounded-[3px] overflow-hidden flex flex-col relative before:absolute before:-inset-1 before:bg-gradient-to-r before:from-transparent before:via-white/5 before:to-transparent before:-translate-x-full hover:before:animate-[shimmer_1.5s_infinite] transition-transform hover:-translate-y-1 shadow">
      ...
      <Link href={`/guides/${guide.slug}`} className="block relative aspect-video overflow-hidden border-b border-[#1e293b] bg-[#050a14]">
        {guide.thumbnail_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-white/20 text-xs font-black uppercase">
            RESİM YOK
          </div>
        )}

        {/* Category Badge */}
        <div className="absolute top-3 left-3 bg-[#facc15] text-black text-[10px] font-black px-2 py-0.5 rounded-[2px] uppercase tracking-wider border border-[#1e293b] shadow">
          {guide.category.name}
        </div>
      </Link>
      ...
      <div className="p-4 flex flex-col flex-1">
        <Link href={`/guides/${guide.slug}`} className="block mb-2">
          <h3 className="text-sm font-black text-white group-hover:text-[#facc15] transition-colors leading-tight line-clamp-2 uppercase tracking-tight">
            {guide.title}
          </h3>
        </Link>
        <p className="text-xs text-gray-300 font-medium line-clamp-2 mb-4 flex-1">
          {guide.summary}
        </p>

        {/* Tags Row */}
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <div className={`flex items-center gap-1 text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-[2px] border shadow-sm ${diffColorClass}`}>
            <BarChart2 size={12} /> {difficulty}
          </div>
          {guide.custom_data?.estimated_time && (
            <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-[2px] border border-[#1e293b] bg-[#050a14] text-gray-300 shadow-sm">
              <Clock size={12} /> {guide.custom_data.estimated_time}
            </div>
          )}
          {guide.custom_data?.reward_badge && (
             <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-[2px] border border-purple-500/50 bg-purple-500/20 text-purple-300 shadow-sm">
                ⭐ {guide.custom_data.reward_badge}
             </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-[#1e293b] mt-auto">
          {guide.author ? (
            ...
            <span className="text-xs font-black uppercase tracking-tight text-gray-400">SİSTEM</span>
          )}
